...
async function sendMessage() {
    ...
    try {
        const res = await fetch('http://localhost:5001/api/ai_query', {  // <-- بورت الخدمة الجديدة
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: text /*, search: "optional" */ })
        });
        ...
        if (data && data.reply) {
            ...
            appendMessage('bot', 'خطأ: ' + data.error);
        } else {
            appendMessage('bot', 'معلش، مفيش رد من السيرفر.');
        }
    } catch (err) {
        console.error(err);
